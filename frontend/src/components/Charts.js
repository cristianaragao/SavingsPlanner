...
export default function Chartjs() {
  const { data, pieData, lineData } = useData();
      ...
      >
          ...
          <div className="absolute left-2/4 top-2/4 -translate-x-2/4 -translate-y-2/4 h-56 w-56 md:h-[10.5rem] md:w-[10.5rem] lg:h-56 lg:w-56 shadow-lg1 rounded-full z-10">
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <p className="text-lg font-semibold color-gray3 my-0">
                {formatterBRL(data.accumulatedValue)}
              </p>
              <p className="text-gray3 mt-2 mb-0">Você terá poupado:</p>
              <p className="font-medium">{formatterBRL(data.spare)}</p>
            </div>
          </div>
          <PieChart />
        </div>
        ...
        >
          {pieData
            ...
            .map((item, index) => (
              <div key={index} className="flex flex-row  items-end justify-between w-5/6 mx-auto">
                ...
                <p className="text-black3">{formatterPercent(item.percent * 100)}</p>
              </div>
            ))}
        </div>
      </div>
